...
Проект реализует концепцию защищенной оверлейной сети с некоторым уклоном в сторону SD-WAN. В отличие от обычного VPN, где весь трафик заворачивается в одну трубу, я использую гибридную схему с балансировкой и PBR. Трафик классифицируется на уровне домашнего роутера и хаба, выбирая оптимальный маршрут.
...
graph LR
    ...
    subgraph Edge_Layer ["🏠 Клиенты"]
        direction TB
        Mobile["📱 Mobile Clients<br/>(Roaming)"]:::device
        
        UDR7["🛡️ Home Ubiquiti Dream Router 7<br/>(IPS/IDS)"]:::router
        
        DoH["🔒 DNS-over-HTTPS<br/>(Control D)"]
        
        %% Связь внутри дома
        UDR7 --"Secure DNS"--> DoH
    end

    InternetRU(("🇷🇺 RU/Local Internet")):::net

    subgraph Core_Hub ["🇷🇺 cloud.ru"]
        direction TB
    ...
    subgraph Transport_Layer ["☁️ Transport Masking"]
        CDN["🌍 Cloudflare CDN"]
    end

    subgraph Exit_Nodes ["🇪🇺 Exit Nodes"]
        direction TB
        Exit1["Nginx / VPS Netherlands)"]
        Exit2["Nginx / VPS Israel"]
    end
    
    InternetGlobal(("🌐 Global Internet")):::net
